...
export class SchemaSerializer<T> implements Serializer<T> {
  ...
  public reset(newState: T & Schema) {
    this.state = newState;
    this.useFilters = (Reflection as any).hasFilter(newState.constructor as typeof Schema);
  }

  // @ts-ignore
  public getFullState(client?: Client) {
    const fullEncodedState = (this.state as any).encodeAll(this.useFilters);
    fullEncodedState.unshift(Protocol.ROOM_STATE);

    if (client && this.useFilters) {
      return (this.state as any).applyFilters(client, true);
  ...
  public applyPatches(clients: Client[]) {
    ...
    if (hasChanges) {
      ...
      // get patch bytes
      const patches = (this.state as any).encode(false, [], this.useFilters);
      ...
      if (!this.useFilters) {
        ...
        while (numClients--) {
          ...
          //
          if (client.state === ClientState.JOINED) {
            const filteredPatches = (this.state as any).applyFilters(client);
            client.raw(Buffer.from([Protocol.ROOM_STATE_PATCH, ...filteredPatches]) as any);
          }
        }

        (this.state as any).discardAllChanges();
      }
  ...
  public handshake() {
    ...
    if (!this.handshakeCache) {
      this.handshakeCache = (this.state && Reflection.encode(this.state as any));
    }

    return this.handshakeCache as any;
  }
