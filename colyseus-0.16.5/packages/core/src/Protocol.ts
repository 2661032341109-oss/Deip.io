...
export const getMessageBytes = {
  [Protocol.JOIN_ROOM]: (reconnectionToken: string, serializerId: string, handshake?: Buffer) => {
    ...
    encode.utf8Write(packr.buffer as any, reconnectionToken, it);

    packr.buffer[it.offset++] = Buffer.byteLength(serializerId, "utf8");
    encode.utf8Write(packr.buffer as any, serializerId, it);

    let handshakeLength = handshake?.byteLength || 0;

    // check if buffer needs to be resized
    if (handshakeLength > packr.buffer.byteLength - it.offset) {
      packr.useBuffer(Buffer.alloc(it.offset + handshakeLength, packr.buffer as unknown as Buffer));
    }
    ...
    return Buffer.from(packr.buffer.subarray(0, it.offset + handshakeLength) as unknown as Buffer);
  },
  ...
  [Protocol.ERROR]: (code: number, message: string = '') => {
    ...
    encode.number(packr.buffer as any, code, it);
    encode.string(packr.buffer as any, message, it);

    return Buffer.from(packr.buffer.subarray(0, it.offset) as unknown as Buffer);
  },
  ...
  raw: (code: Protocol, type: string | number, message?: any, rawMessage?: Uint8Array | Buffer) => {
    ...
    if (typeof (type) === 'string') {
      encode.string(packr.buffer as any, type as string, it);

    } else {
      encode.number(packr.buffer as any, type, it);
    }
    ...
    if (message !== undefined) {
      ...
      if (process.env.NODE_ENV !== "production") {
        packr.useBuffer(packr.buffer as any);
      }
      ...
      return Buffer.from(packr.buffer.subarray(0, endOfBufferOffset) as unknown as Buffer);
      ...
      if (rawMessage.length + it.offset > packr.buffer.byteLength) {
        packr.useBuffer(Buffer.alloc(it.offset + rawMessage.length, packr.buffer as any));
      }

      // copy raw message into packr.buffer
      packr.buffer.set(rawMessage, it.offset);

      return Buffer.from(packr.buffer.subarray(0, it.offset + rawMessage.byteLength) as unknown as Buffer);
